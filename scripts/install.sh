...
DEFAULT_TAGLINE="All your chats, one Mouse."
NODE_MIN_MAJOR=22
NODE_MIN_MINOR=12
...
GUM_VERSION="${MOUSE_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🐭 Mouse Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://mouseplatform.com/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$MOUSE_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.mouseplatform.com/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_mouse_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/mouse" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/mouse"|"/usr/local/bin/mouse")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/mouse/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale mouse symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing mouse binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Mouse package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_mouse_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Mouse npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY.*rename .*mouse" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_mouse_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_mouse_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Mouse npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an mouse binary already exists"
            if [[ -n "$conflict" ]]; then
...
map_legacy_env "MOUSE_TAGLINE_INDEX" "CLAWDBOT_TAGLINE_INDEX"
map_legacy_env "MOUSE_NO_ONBOARD" "CLAWDBOT_NO_ONBOARD"
map_legacy_env "MOUSE_NO_PROMPT" "CLAWDBOT_NO_PROMPT"
map_legacy_env "MOUSE_DRY_RUN" "CLAWDBOT_DRY_RUN"
map_legacy_env "MOUSE_INSTALL_METHOD" "CLAWDBOT_INSTALL_METHOD"
map_legacy_env "MOUSE_VERSION" "CLAWDBOT_VERSION"
map_legacy_env "MOUSE_BETA" "CLAWDBOT_BETA"
map_legacy_env "MOUSE_GIT_DIR" "CLAWDBOT_GIT_DIR"
...
pick_tagline() {
    ...
    if [[ -n "${MOUSE_TAGLINE_INDEX:-}" ]]; then
        if [[ "${MOUSE_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((MOUSE_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${MOUSE_NO_ONBOARD:-0}
NO_PROMPT=${MOUSE_NO_PROMPT:-0}
DRY_RUN=${MOUSE_DRY_RUN:-0}
SILENT_ONBOARD=${MOUSE_SILENT:-0}
INSTALL_METHOD=${MOUSE_INSTALL_METHOD:-}
MOUSE_VERSION=${MOUSE_VERSION:-latest}
USE_BETA=${MOUSE_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/.mouse"
GIT_DIR=${MOUSE_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${MOUSE_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${MOUSE_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${MOUSE_VERBOSE:-0}"
MOUSE_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Mouse installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://mouseplatform.com/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/.mouse)
...
Environment variables:
  MOUSE_INSTALL_METHOD=git|npm
  MOUSE_VERSION=latest|next|<semver>
  MOUSE_BETA=0|1
  MOUSE_GIT_DIR=...
  MOUSE_GIT_UPDATE=0|1
  MOUSE_NO_PROMPT=1
  MOUSE_DRY_RUN=1
  MOUSE_NO_ONBOARD=1
  MOUSE_VERBOSE=1
  MOUSE_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://mouseplatform.com/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://mouseplatform.com/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://mouseplatform.com/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Mouse checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Mouse source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_mouse_checkout() {
    ...
    if ! grep -q '"name": "mouse"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://mouseplatform.com/install.sh | bash"
}
...
resolve_mouse_bin() {
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/mouse" ]]; then
        echo "${npm_bin}/mouse"
        return 0
    fi
...
install_openclaw_from_git() {
    ...
    cat > "$HOME/.local/bin/mouse" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/mouse"
    ui_success "Mouse wrapper installed to \$HOME/.local/bin/mouse"
...
install_openclaw() {
    ...
    if [[ "${MOUSE_VERSION}" == "latest" ]]; then
        ...
        install_spec="${package_name}@${MOUSE_VERSION}"
    fi

    if ! install_mouse_npm "${install_spec}"; then
...
run_doctor() {
    ...
    if [[ -z "$claw" ]]; then
        claw="$(resolve_mouse_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (mouse not on PATH yet)"
        warn_mouse_not_found
        return 0
    fi
...
run_bootstrap_onboarding_if_needed() {
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run mouse onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but mouse not on PATH; skipping onboarding"
        warn_mouse_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run mouse onboard to retry"
        return
    }
}
...
resolve_openclaw_version() {
    ...
    local claw="${MOUSE_BIN:-}"
    if [[ -z "$claw" ]] && command -v mouse &> /dev/null; then
        claw="$(command -v mouse)"
    fi
    if [[ -n "$claw" ]]; then
...
main() {
    ...
    detected_checkout="$(detect_mouse_checkout "$PWD" || true)"
        ...
        if ! is_promptable; then
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set MOUSE_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    MOUSE_BIN="$(resolve_mouse_bin || true)"
    ...
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🐭 Mouse installed successfully (${installed_version})!"
    else
        ui_celebrate "🐭 Mouse installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running mouse doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                MOUSE_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                MOUSE_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                MOUSE_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run mouse doctor and mouse plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (mouse not on PATH yet)"
                    warn_mouse_not_found
                    return 0
                fi
                if [[ "$SILENT_ONBOARD" == "1" ]]; then
                    exec "$claw" onboard --silent
                else
                    exec </dev/tty
                    exec "$claw" onboard
                fi
            fi
            ui_info "No TTY; run mouse onboard to finish setup"
            return 0
        fi
    fi
    ...
    if command -v mouse &> /dev/null; then
        ...
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (mouse daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if MOUSE_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: mouse daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${MOUSE_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
