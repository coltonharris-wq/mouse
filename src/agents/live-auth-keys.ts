...
const GOOGLE_LIVE_SINGLE_KEY = "MOUSE_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "MOUSE_LIVE_ANTHROPIC_KEY",
    listVar: "MOUSE_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "MOUSE_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `MOUSE_LIVE_${base}_KEY`;
