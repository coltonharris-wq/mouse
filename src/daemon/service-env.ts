...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.MOUSE_PROFILE;
  ...
  return {
    ...
    MOUSE_GATEWAY_PORT: String(port),
    MOUSE_LAUNCHD_LABEL: resolvedLaunchdLabel,
    MOUSE_SYSTEMD_UNIT: systemdUnit,
    MOUSE_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    MOUSE_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    MOUSE_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    MOUSE_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.MOUSE_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    MOUSE_GATEWAY_TOKEN: gatewayToken,
    MOUSE_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    MOUSE_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    MOUSE_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    MOUSE_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    MOUSE_LOG_PREFIX: "node",
    MOUSE_SERVICE_MARKER: NODE_SERVICE_MARKER,
    MOUSE_SERVICE_KIND: NODE_SERVICE_KIND,
    MOUSE_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    MOUSE_STATE_DIR: sharedEnv.stateDir,
    MOUSE_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.MOUSE_STATE_DIR;
  const configPath = env.MOUSE_CONFIG_PATH;
