...
export function resolveConfigDir(
  ...
  const override = env.MOUSE_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.MOUSE_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$MOUSE_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via MOUSE_STATE_DIR.
