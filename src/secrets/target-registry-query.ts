...
const MOUSE_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of MOUSE_COMPILED_SECRET_TARGETS) {
...
const MOUSE_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: MOUSE_COMPILED_SECRET_TARGETS,
    entriesById: MOUSE_TARGETS_BY_ID,
  });
