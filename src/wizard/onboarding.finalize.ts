  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.openclaw/openclaw.json (gateway.auth.token) or MOUSE_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("mouse config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("mouse doctor --generate-gateway-token")}`,
        "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
        `Open the dashboard anytime: ${formatCliCommand("mouse dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("mouse dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
    ...
    if (webSearchEnabled !== false && hasKey) {
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("mouse configure --section web")}`,
          "",
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("mouse configure --section web")}`,
          "",
          "Docs: https://docs.openclaw.ai/tools/web",
        ].join("\n"),
        "Web search",
