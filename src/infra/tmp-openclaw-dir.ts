...
export const POSIX_MOUSE_TMP_DIR = "/tmp/mouse";
/** @deprecated Use POSIX_MOUSE_TMP_DIR */
export const POSIX_OPENCLAW_TMP_DIR = POSIX_MOUSE_TMP_DIR;
...
export function resolvePreferredOpenClawTmpDir(
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "mouse" : `mouse-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[mouse] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const existingPreferredState = resolveDirState(POSIX_MOUSE_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_MOUSE_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_MOUSE_TMP_DIR)) {
      return POSIX_MOUSE_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_MOUSE_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_MOUSE_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_MOUSE_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_MOUSE_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_MOUSE_TMP_DIR;
