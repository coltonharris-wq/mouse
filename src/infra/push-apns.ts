...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.MOUSE_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.MOUSE_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set MOUSE_APNS_TEAM_ID and MOUSE_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.MOUSE_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.MOUSE_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.MOUSE_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set MOUSE_APNS_PRIVATE_KEY_P8 or MOUSE_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading MOUSE_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
