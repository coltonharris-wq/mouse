...
const WINDOWS_TASK_SUPERVISOR_HINT_ENV_VARS = ["MOUSE_WINDOWS_TASK_NAME"] as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.MOUSE_SERVICE_MARKER?.trim();
    const serviceKind = env.MOUSE_SERVICE_KIND?.trim();
