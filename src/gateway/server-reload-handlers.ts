  ...
  const applyHotReload = async (
    ...
    if (plan.restartGmailWatcher) {
      ...
      await startGmailWatcherWithLogs({
        ...
        log: params.logHooks,
        onSkipped: () =>
          params.logHooks.info("skipping gmail watcher restart (MOUSE_SKIP_GMAIL_WATCHER=1)"),
      });
    }

    if (plan.restartChannels.size > 0) {
      if (
        isTruthyEnvValue(process.env.MOUSE_SKIP_CHANNELS) ||
        isTruthyEnvValue(process.env.MOUSE_SKIP_PROVIDERS)
      ) {
        params.logChannels.info(
          "skipping channel reload (MOUSE_SKIP_CHANNELS=1 or MOUSE_SKIP_PROVIDERS=1)",
        );
      } else {
