...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mouse security audit", "Run a local security audit."],
          ["mouse security audit --deep", "Include best-effort live Gateway probe checks."],
          ["mouse security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["mouse security audit --json", "Output machine-readable JSON."],
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      lines.push(muted(`Run deeper: ${formatCliCommand("mouse security audit --deep")}`));

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("mouse security audit --fix")}`));
        if (!fixResult) {
