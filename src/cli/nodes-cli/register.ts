    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mouse nodes status", "List known nodes with live status."],
          ["mouse nodes pairing pending", "Show pending node pairing requests."],
          ['openclaw nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["mouse nodes camera snap --node <id>", "Capture a photo from a node camera."],
