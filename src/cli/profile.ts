...
export function applyCliProfileEnv(params: {
  ...
  env.MOUSE_PROFILE = profile;

  const stateDir = env.MOUSE_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.MOUSE_STATE_DIR?.trim()) {
    env.MOUSE_STATE_DIR = stateDir;
  }

  if (!env.MOUSE_CONFIG_PATH?.trim()) {
    env.MOUSE_CONFIG_PATH = path.join(stateDir, "openclaw.json");
  }

  if (profile === "dev" && !env.MOUSE_GATEWAY_PORT?.trim()) {
    env.MOUSE_GATEWAY_PORT = "19001";
  }
}
