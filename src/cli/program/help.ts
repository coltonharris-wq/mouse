...
const EXAMPLES = [
  ["mouse models --help", "Show detailed help for the models command."],
  [
    "mouse channels login --verbose",
  ...
  ["mouse gateway --port 18789", "Run the WebSocket Gateway locally."],
  ["mouse --dev gateway", "Run a dev Gateway (isolated state/config) on ws://127.0.0.1:19001."],
  ["mouse gateway --force", "Kill anything bound to the default gateway port, then start it."],
  ["mouse gateway ...", "Gateway control via WebSocket."],
  [
...
export function configureProgramHelp(program: Command, ctx: ProgramContext) {
  program
    ...
    .option(
      "--profile <name>",
      "Use a named profile (isolates MOUSE_STATE_DIR/MOUSE_CONFIG_PATH under ~/.openclaw-<name>)",
    )
    .option(
      "--log-level <level>",
