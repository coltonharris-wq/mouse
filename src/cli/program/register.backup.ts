    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mouse backup create", "Create a timestamped backup in the current directory."],
          [
            "mouse backup create --output ~/Backups",
            "Write the archive into an existing backup directory.",
          ],
          [
            "mouse backup create --dry-run --json",
            "Preview the archive plan without writing any files.",
          ],
          [
            "mouse backup create --verify",
            "Create the archive and immediately validate its manifest and payload layout.",
          ],
          [
            "mouse backup create --no-include-workspace",
            "Back up state/config without agent workspace files.",
          ],
          ["mouse backup create --only-config", "Back up only the active JSON config file."],
        ])}`,
    )
    .action(async (opts) => {
  ...
  backup
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "mouse backup verify ./2026-03-09T00-00-00.000Z-openclaw-backup.tar.gz",
            "Check that the archive structure and manifest are intact.",
          ],
          [
            "mouse backup verify ~/Backups/latest.tar.gz --json",
