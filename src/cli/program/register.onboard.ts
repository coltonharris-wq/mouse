...
export function registerOnboardCommand(program: Command) {
  const command = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/onboard", "docs.mouseplatform.com/cli/onboard")}\n`,
    )
    .option("--silent", "Skip ALL prompts and write config directly (King Mouse mode)", false)
    .option("--preset <name>", "Autonomy preset (e.g. king-mouse)")
    .option("--api-key <key>", "Moonshot API key (MOUSE_API_KEY)")
    .option("--model <model>", "Default model (default: moonshot/kimi-k2.5)")
    .option("--port <port>", "Gateway port (default: 3100 in silent mode)")
    .option("--bind <addr>", "Bind address (default: 0.0.0.0 in silent mode)")
    .option("--user-id <id>", "Customer identifier")
    .option("--callback-url <url>", "URL to POST when ready")
    .option("--tailscale-authkey <key>", "Tailscale auth key")
    .option("--auto-skills", "Auto-enable all skills", false)
    .option("--workspace <dir>", "Agent workspace directory (default: ~/.mouse/workspace)")
    .option(
      "--reset",
  ...
  command
    ...
    .option(
      ...
      "Gateway token SecretRef env var name (token auth; e.g. MOUSE_GATEWAY_TOKEN)",
    )
      ...
      await onboardCommand(
        {
          ...
          moonshotApiKey: (opts.apiKey ?? opts.moonshotApiKey) as string | undefined,
