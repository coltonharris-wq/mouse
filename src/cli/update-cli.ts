...
export function registerUpdateCli(program: Command) {
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["mouse update", "Update a source checkout (git)"],
        ["mouse update --channel beta", "Switch to beta channel (git + npm)"],
        ["mouse update --channel dev", "Switch to dev channel (git + npm)"],
        ["mouse update --tag beta", "One-off update to a dist-tag or version"],
        ["mouse update --dry-run", "Preview actions without changing anything"],
        ["mouse update --no-restart", "Update without restarting the service"],
        ["mouse update --json", "Output result as JSON"],
        ["mouse update --yes", "Non-interactive (accept downgrade prompts)"],
        ["mouse update wizard", "Interactive update wizard"],
        ["mouse --update", "Shorthand for openclaw update"],
      ] as const;
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.openclaw.ai/cli/update")}`;
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mouse update status", "Show channel + version status."],
          ["mouse update status --json", "JSON output."],
          ["mouse update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
