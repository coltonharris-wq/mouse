...
type HomeEnvSnapshot = {
  ...
  MOUSE_STATE_DIR: string | undefined;
  MOUSE_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    MOUSE_STATE_DIR: process.env.MOUSE_STATE_DIR,
    MOUSE_AGENT_DIR: process.env.MOUSE_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    process.env.MOUSE_STATE_DIR = path.join(home, ".openclaw");
    process.env.MOUSE_AGENT_DIR = path.join(home, ".openclaw", "agent");
