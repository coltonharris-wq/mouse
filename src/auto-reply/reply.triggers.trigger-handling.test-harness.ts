...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    openclawHome: process.env.MOUSE_HOME,
    stateDir: process.env.MOUSE_STATE_DIR,
  };
}
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("MOUSE_HOME", snapshot.openclawHome);
  restoreKey("MOUSE_STATE_DIR", snapshot.stateDir);
}

function setTempHomeEnv(home: string): void {
  process.env.HOME = home;
  process.env.USERPROFILE = home;
  delete process.env.MOUSE_HOME;
  process.env.MOUSE_STATE_DIR = join(home, ".openclaw");
