...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  if (prevGatewayPort === undefined) {
    delete process.env.MOUSE_GATEWAY_PORT;
    return;
  }
  process.env.MOUSE_GATEWAY_PORT = prevGatewayPort;
}
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.MOUSE_GATEWAY_PORT;
  process.env.MOUSE_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.MOUSE_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.MOUSE_GATEWAY_PASSWORD;
  delete process.env.MOUSE_GATEWAY_TOKEN;
  delete process.env.MOUSE_GATEWAY_PASSWORD;
}
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.MOUSE_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.MOUSE_GATEWAY_PASSWORD;
  } else {
    process.env.MOUSE_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
