...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set MOUSE_GATEWAY_TOKEN/MOUSE_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.MOUSE_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.MOUSE_GATEWAY_PASSWORD);
