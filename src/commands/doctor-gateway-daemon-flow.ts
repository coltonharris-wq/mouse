...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.MOUSE_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("mouse gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
