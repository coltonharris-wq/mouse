...
async function runGatewayHealthCheck(params: {
  ...
  const token =
    process.env.MOUSE_GATEWAY_TOKEN ?? process.env.CLAWDBOT_GATEWAY_TOKEN ?? configuredToken;
  const password =
    process.env.MOUSE_GATEWAY_PASSWORD ??
...
export async function runConfigureWizard(
  ...
  try {
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("mouse doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token:
        process.env.MOUSE_GATEWAY_TOKEN ??
