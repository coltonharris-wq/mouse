...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("mouse models auth setup-token")}\` or \`${formatCliCommand("mouse configure")}\`.`
          : `Run \`${formatCliCommand("mouse configure")}\` or set an API key env var.`;
