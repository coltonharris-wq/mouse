...
export async function setupAuthTestEnv(
  ...
  process.env.MOUSE_STATE_DIR = stateDir;
  process.env.MOUSE_AGENT_DIR = agentDir;
...
export function requireOpenClawAgentDir(): string {
  const agentDir = process.env.MOUSE_AGENT_DIR;
  if (!agentDir) {
    throw new Error("MOUSE_AGENT_DIR not set");
  }
  return agentDir;
}
