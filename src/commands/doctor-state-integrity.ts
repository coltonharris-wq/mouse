...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for MOUSE_STATE_DIR.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // MOUSE_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        `  Set locally: MOUSE_STATE_DIR=~/.openclaw ${formatCliCommand("mouse doctor")}`,
      ].join("\n"),
    );
  }
