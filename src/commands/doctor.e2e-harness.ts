...
beforeEach(() => {
  ...
  setStdinTty(true);
  originalStateDir = process.env.MOUSE_STATE_DIR;
  originalUpdateInProgress = process.env.MOUSE_UPDATE_IN_PROGRESS;
  process.env.MOUSE_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openclaw-doctor-state-"));
  process.env.MOUSE_STATE_DIR = tempStateDir;
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    delete process.env.MOUSE_STATE_DIR;
  } else {
    process.env.MOUSE_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.MOUSE_UPDATE_IN_PROGRESS;
  } else {
    process.env.MOUSE_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
