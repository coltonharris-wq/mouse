...
export async function noteSecurityWarnings(cfg: OpenClawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("mouse security audit --deep")}`;
  ...
  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      ...
      `  Check local policy with: ${formatCliCommand("mouse approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("mouse configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("mouse config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("mouse doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "mouse config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("mouse config set gateway.bind loopback")}`,
