...
export async function statusCommand(
  ...
  if (pairingRecovery) {
    ...
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("mouse devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("mouse devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("mouse security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("mouse security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("mouse status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("mouse logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("mouse status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("mouse gateway probe")}`);
  }
}
